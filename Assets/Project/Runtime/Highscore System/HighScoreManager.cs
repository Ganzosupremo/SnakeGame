...
namespace SnakeGame.HighscoreSystem
{
    public class HighScoreManager : SingletonMonoBehaviour<HighScoreManager>, IPersistenceData
    {
        ...
        private static HighScore _highScore = new();
        ...
        public static void AddScore(int rank, Score score)
        {
            _highScore.ScoreList.Insert(rank - 1, score);

            if (_highScore.ScoreList.Count > Settings.maxNumberOfHighScoresToSave)
            {
                _highScore.ScoreList.RemoveAt(Settings.maxNumberOfHighScoresToSave);
            }
