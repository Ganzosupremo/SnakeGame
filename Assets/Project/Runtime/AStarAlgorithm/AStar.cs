...
namespace SnakeGame.AStarPathfinding
{
    ...
    public static class AStar
    {
        ...
        public static async UniTask<Stack<Vector3>> BuildPathAsync(Room room, Vector3Int startGridPosition, Vector3Int endGridPosition, CancellationToken token = default)
        {
            try
            {
                // Adjust positions by the lower bounds
                ...
                AStarNode endPathNode = await FindShortestPathAsync(startNode, endNode, gridNode, openHeapNodeList, closedNodeHashset, room.InstantiatedRoom);

                if (endPathNode != null)
                {
                    return await CreatePathStackAsync(endPathNode, room);
                }
        ...
        private static async UniTask<AStarNode> FindShortestPathAsync(AStarNode startNode, AStarNode endNode, GridNode gridNode, Heap<AStarNode> openNodeHeap, HashSet<AStarNode> closedNodeHashset, InstantiatedRoom instantiatedRoom, CancellationToken token = default)
        {
            return await Task.Run(() =>
            {
                ...
                // Call the synchronous FindShortestPath method here, so it runs on a separate thread
                return FindShortestPath(startNode, endNode, gridNode, openNodeHeap, closedNodeHashset, instantiatedRoom);
            });
        }
        ...
        private async static UniTask<Stack<Vector3>> CreatePathStackAsync(AStarNode targetNode, Room room)
        {
            Stack<Vector3> movementPathStack = new();
