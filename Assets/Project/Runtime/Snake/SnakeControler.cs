...
namespace SnakeGame.PlayerSystem
{
    ...
    {
        public bool IsSnakeEnabled { get => _isSnakeEnabled; }

        [SerializeField] private MovementDetailsSO movementDetails;
        [SerializeField] private Transform weaponShootPosition;

        private Snake snake;
        private SnakeControl _snakeInputActions;
        ...
        private bool _isSnakeEnabled = false;

        private void Awake()
        {
            _snakeInputActions = new();
        ...
        private void Start()
        {
            ...
            _snakeInputActions.Snake.Enable();
            _isSnakeEnabled = false;
        ...
        private void OnMove()
        {
            ...
            moveDirection = _snakeInputActions.Snake.Move.ReadValue<Vector2>();
            ...
            if (moveDirection != Vector2.zero)
            {
                ...
                snake.RotateBody(moveDirection);
            }
            else
            {
        ...
        private void OnSpecialAbility()
        {
            ...
            if (_snakeInputActions.Snake.SpecialAbility.IsPressed() && 
        ...
        public void OnReload()
        {
            ...
            if (_snakeInputActions.Snake.Reload.WasPressedThisFrame())
        ...
        private void OnSwitchWeapon()
        {
            ...
            Vector2 switchWeaponInput = _snakeInputActions.Snake.SwitchWeapons.ReadValue<Vector2>();
            ...
            if (_snakeInputActions.Snake.SetWeaponAtFirstPos.WasPressedThisFrame())
                SetWeaponFirstInList();
        }

        private void CommandWindowControl()
        {
            if (_snakeInputActions.Snake.CheatWindow.WasPressedThisFrame())
                DebugController.Instance.OnToggleDebug();

            if (_snakeInputActions.Snake.EnterCommand.WasPressedThisFrame())
        ...
        private async UniTask TeleportSnake(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                UpdateSnakeBodiesPosition(currentRoom);
        ...
        private void UpdateSnakeBodiesPosition(Room currentRoom)
        {
            foreach (SnakeBody snakeBody in snake.SnakeBodyList)
            {
                snakeBody.transform.position = new Vector3((currentRoom.lowerBounds.x + currentRoom.upperBounds.x) / 2f,
                    (currentRoom.lowerBounds.y + currentRoom.upperBounds.y) / 2f, 0f);

                snakeBody.transform.position = HelperUtilities.GetNearestSpawnPointPosition(snake.gameObject.transform.position);
            }

            //Vector3 prevPosition = snake.gameObject.transform.position;
            //foreach (SnakeBody snakeBody in snake.SnakeBodyList)
            //{
            //    Vector3 currentPosition = snakeBody.transform.position;
            //    snakeBody.SetTargetPosition(prevPosition);
        ...
        {
            if (_snakeInputActions.Snake.Fire.IsPressed())
            {
        ...
        {
            _isSnakeEnabled = value;
        }

        public void EnableSnake()
        {
            _isSnakeEnabled = true;
            //_snakeInputActions.Enable();
        }
