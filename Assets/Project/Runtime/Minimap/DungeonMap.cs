...
namespace SnakeGame.Minimap
{
    public class DungeonMap : SingletonMonoBehaviour<DungeonMap>
    {
        ...
        private IEnumerator TeleportToRoom(Vector3 worldPosition, Room room)
        {
            ...
            TeleportSnakeBodiesToRoom(nearestSpawnPosition);
        ...
        private void TeleportSnakeBodiesToRoom(Vector3 nearestSpawnPosition)
        {
            foreach (var body in GameManager.Instance.GetSnake().SnakeBodyList)
            {
                body.transform.position = nearestSpawnPosition;
            }
        }
